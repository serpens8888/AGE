...
create_descriptors :: proc(device: vk.Device) -> (manager: Descriptor_Manager, err: Error) {
	...
	vk.CreateDescriptorSetLayout(device, &layout_info, nil, &manager.layout) or_return
	...
	vk.AllocateDescriptorSets(device, &alloc_info, raw_data(manager.sets[:])) or_return
...
add_sampled_image :: proc(
	...
	set := manager.sets[state.current_frame]
