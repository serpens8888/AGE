...
create_swapchain :: proc(
    ...
    image_count: u32 = 2
    if(capabilities.minImageCount == capabilities.maxImageCount || capabilities.minImageCount > image_count){
    ...
    if(sharing_mode == .CONCURRENT){
        ...
        assert(len(distinct_families) > 0, "must pass GPU_Queues into create_swapchain if sharing_mode is set to .CONCURRENT")
...
select_swapchain_format :: proc(gpu: vk.PhysicalDevice, surface: vk.SurfaceKHR, requested: vk.SurfaceFormatKHR) -> (surface_format: vk.SurfaceFormatKHR, err: mem.Allocator_Error){
    ...
    vk.GetPhysicalDeviceSurfaceFormatsKHR(gpu, surface, &format_count, raw_data(formats))
...
select_swapchain_present_mode :: proc(gpu: vk.PhysicalDevice, surface: vk.SurfaceKHR, requested: vk.PresentModeKHR) -> (present_mode: vk.PresentModeKHR, err: mem.Allocator_Error){

   	present_mode_count: u32
	vk.GetPhysicalDeviceSurfacePresentModesKHR(gpu, surface, &present_mode_count, nil)

	present_modes := make([]vk.PresentModeKHR, present_mode_count) or_return
    defer delete(present_modes)

	vk.GetPhysicalDeviceSurfacePresentModesKHR(gpu, surface, &present_mode_count, raw_data(present_modes))
...
get_swapchain_extent :: proc(capabilities: vk.SurfaceCapabilitiesKHR, window: ^sdl.Window) -> vk.Extent2D{
