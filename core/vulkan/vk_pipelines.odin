...
create_shader_module :: proc(device: vk.Device, shader_path: string) -> (mod: vk.ShaderModule, err: Error){
...
create_graphics_pipeline :: proc(device: vk.Device, graphics_mod: Graphics_Module, layout: vk.PipelineLayout, shader_mod: vk.ShaderModule) -> (pipeline: vk.Pipeline, err: Error){
    ...
    scissor: vk.Rect2D = {
        offset = {x = 0, y = 0},
        extent = {width = graphics_mod.swapchain.extent.width, height = graphics_mod.swapchain.extent.height},
    }
    ...
    check_vk(vk.CreateGraphicsPipelines(device, 0, 1, &pipeline_info, nil, &pipeline)) or_return
