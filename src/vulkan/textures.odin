...
create_image :: proc(ci : ^ImageInfo, allocator : vma.Allocator) -> Image {
    ...
    if (vma.create_image(allocator, image_info, aci, &image, &mem, {}) !=
           .SUCCESS) {
...
transition_image_layout :: proc(
    ...
    case .UNDEFINED:
        {barrier.srcAccessMask = {};src_stage = {.TOP_OF_PIPE}}
    case .TRANSFER_DST_OPTIMAL:
        {barrier.srcAccessMask = {.TRANSFER_WRITE};src_stage = {.TRANSFER}}
    }

    #partial switch new_layout {
    case .TRANSFER_DST_OPTIMAL:
        {barrier.dstAccessMask = {.TRANSFER_WRITE};dst_stage = {.TRANSFER}}
    case .SHADER_READ_ONLY_OPTIMAL:
        {barrier.dstAccessMask = {.SHADER_READ};dst_stage = {.FRAGMENT_SHADER}}
    }

    if ((barrier.srcAccessMask == {} && src_stage == {}) ||
           (barrier.dstAccessMask == {} && dst_stage == {})) {
...
create_sampler :: proc(
    ...
    if (vk.CreateSampler(device, &sampler_info, nil, &sampler.handle) !=
           .SUCCESS) {
...
create_texture_array :: proc(
    ...
    size = align_size(
        size,
        ctx.gpu_properties.descriptor_buffer.descriptorBufferOffsetAlignment,
    )
    ...
    for &image, i in textures {
        ...
        offset :=
            vk.DeviceSize(
                i *
                ctx.gpu_properties.descriptor_buffer.sampledImageDescriptorSize,
            ) +
            image_offset
    ...
    for &sampler, i in samplers {
        ...
        offset :=
            vk.DeviceSize(
                i * ctx.gpu_properties.descriptor_buffer.samplerDescriptorSize,
            ) +
            sampler_offset
        dst := rawptr(uintptr(mapped_ptr) + uintptr(offset))
        vk.GetDescriptorEXT(
            ctx.device,
            &descriptor_info,
            ctx.gpu_properties.descriptor_buffer.samplerDescriptorSize,
            dst,
        )
